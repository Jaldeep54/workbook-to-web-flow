...
export function LabelOrderSuggestionTab({
  ...
  const placeOrders = useMutation({
    mutationFn: async (chosenOrderDate: string) => {
      ...
      const nameByShopId = new Map(shopsToOrder.map((g) => [g.shopId, g.shopName]));

      const orders = shopsToOrder
        .map((g) => ({
          shop_id: g.shopId,
          lines: labelProducts
            .map((lp) => {
              ...
              return qty > 0 ? { label_product_id: lp.id, sheets: qty } : null;
            })
            .filter((l): l is { label_product_id: string; sheets: number } => l !== null),
        }))
        .filter((entry) => entry.lines.length > 0);

      if (orders.length === 0) throw new Error("Nothing selected to order");

      // One request places the whole selection. The API attempts each shop
      // independently and reports per shop, so one failure doesn't discard the
      // rest of the batch.
      const result = await labelOrdersApi.createBulk({ order_date: chosenOrderDate, orders });

      return {
        successes: result.successes.map((s) => s.shop_id),
        failures: result.failures.map((f) => ({
          shopName: nameByShopId.get(f.shop_id) ?? f.shop_id,
          message: f.message,
        })),
        orderDate: chosenOrderDate,
      };
    },
