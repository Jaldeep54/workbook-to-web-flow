...
import { deliveriesApi } from "@/services/klinzo.service";
import { productsQuery, shopsQuery } from "@/lib/queries";
import { deliveriesQuery, pendingOrdersQuery } from "@/lib/records";
import {
  DELIVERY_STATUSES,
...
export const Route = createFileRoute("/_authenticated/deliveries")({
  component: () => (
    <RequirePermission resource={RESOURCES.deliveries}>
      <DeliveriesPage />
    </RequirePermission>
  ),
});
...
function DeliveriesPage() {
  ...
  // recorded against.
  const pending = useQuery(pendingOrdersQuery);
  ...
  /**
   * Recording a delivery goes through the API's order lifecycle: it copies the
   * order's quantities onto the delivery, computes the money figures from the
   * current rates and raises the payment — the same path the Delivery Sheet's
   * "Delivered" toggle uses, so both produce identical records.
   */
        ...
        actions={
          <>
            ...
            <Dialog open={open} onOpenChange={setOpen}>
              <Can resource={RESOURCES.deliveries} action="create">
              ...
              <DialogContent className="max-w-xl">
                ...
                <div>
                  <p className="mb-2 text-xs font-medium uppercase tracking-wide text-muted-foreground">
                    Quantities from the order
                  </p>
                  <ProductQtyGrid products={products} value={qty} onChange={setQty} readOnly />
                  <p className="mt-2 text-xs text-muted-foreground">
                    Delivered quantities always match the order. Edit the order itself to change
                    them — the delivery and its payment then follow automatically.
                  </p>
                </div>
