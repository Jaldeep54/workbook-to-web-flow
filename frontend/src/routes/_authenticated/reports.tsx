...
import { productsQuery, summaryByAreaQuery, type DashboardSummary } from "@/lib/queries";
...
function ReportsPage() {
  ...
  // One summary per month in the range — the same endpoint the Overview uses.
  const results = useQueries({ queries: months.map((m) => summaryByAreaQuery(m, "all")) });
  const loading = results.some((r: { isLoading: boolean }) => r.isLoading);
