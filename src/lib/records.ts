...
type ShopRef = { code: string; shop_name: string } | null;
...
export const ordersQuery = (month: string, shopId: string | "all") =>
  queryOptions({
    ...
    queryFn: async () => {
      let q = supabase
        .from("orders")
        .select("id, shop_id, order_no, order_date, total_qty, notes, shops(code, shop_name), order_lines(product_id, qty)")
        .eq("month", month)
  ...
  queryOptions({
    ...
    queryFn: async () => {
      let q = supabase
        .from("deliveries")
        .select(
          "id, shop_id, order_id, delivery_date, status, total_qty, total_sales, labelling_cost, packaging_cost, production_cost, total_fixed_cost, profit, shops(code, shop_name), orders(order_no), delivery_lines(product_id, qty)",
        )
        .eq("month", month)
...
export const paymentsQuery = (month: string, shopId: string | "all") =>
  queryOptions({
    ...
    queryFn: async () => {
      let q = supabase
        .from("payments")
        .select("id, shop_id, order_id, payment_date, status, collected_by, amount, shops(code, shop_name), orders(order_no)")
        .eq("month", month)
...
export const labelOrdersQuery = (month: string, shopId: string | "all") =>
  queryOptions({
    ...
    queryFn: async () => {
      let q = supabase
        .from("label_orders")
        .select("id, shop_id, order_no, order_date, total_labels, shops(code, shop_name), label_order_lines(label_product_id, sheets, products)")
        .eq("month", month)
        .order("order_date", { ascending: false })
        .limit(1000);
      if (shopId !== "all") q = q.eq("shop_id", shopId);
