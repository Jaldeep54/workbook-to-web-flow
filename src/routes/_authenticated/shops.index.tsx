...
import { deleteShopImage, isHeicPath, uploadShopImage } from "@/lib/shop-image";
...
export const Route = createFileRoute("/_authenticated/shops/")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Every Klinzo shop with contact details, handler and design type.",
      },
...
function ShopsPage() {
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      const shopId = editing?.id ?? crypto.randomUUID();

      let imagePath: string | null | undefined = undefined; // undefined = leave unchanged
      if (imageFile) {
        imagePath = await uploadShopImage(shopId, imageFile);
        if (editing?.image_path) void deleteShopImage(editing.image_path);
      } else if (removeImage && editing?.image_path) {
        await deleteShopImage(editing.image_path);
        imagePath = null;
      }
      ...
      if (editing) {
        ...
        const { error } = await supabase.from("shops").insert({ ...payload, id: shopId });
        if (error) throw new Error(error.message);
      }
...
function ShopDetailsDialog({
  ...
  const imageQuery = useQuery({
    ...shopImageUrlQuery(shop?.image_path ?? null),
    enabled: !!shop?.image_path,
  });
